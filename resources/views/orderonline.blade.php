...
<head>
    ...
    @endif
    <meta name="theme-color" content="#F7DBAF">
    <!-- Open Graph / Facebook -->
    ...
    <link rel="manifest" href="{{ asset('js/yummy-bites/manifest.json') }}">
...
<body class="bgb5">
            ...
            <div>
                <tbl_nm class="p10 @if ($business->lang == 'ar') r rtl @else l @endif">
                    <cl class=" w10 m_w20 m_m_w33 vt">
                        <div class="ov round100 w100 sq100 bgb10 sh10 ba p5"
                            style="top:-40px;background-color: #F7DBAF;">
                            <div class="cc cov  w90 sq90 round100"
                                style="background-image: url('http://www.qr.algorexe.com/images/{{ $business->image }}');">
                            </div>
                        </div>
                    </cl>
        ...
        <div class="mt20 ph20 m_ph10">
            <div class="fs0 ph10 main-carousel ">
                ...
                @foreach ($categories as $category)
                    @if ($loop->first)
                        <div class="p5 g_nm">
                            <div class="pv10 ph15 fs14 round3x sh10  bgw  tablinks active " id="{{ $category->id }}"
                                data-index=@php echo($counter) @endphp>
                                {{ $category->name }}</div>
                        </div>
                        @php $counter++; @endphp
                    @else
                        <div class="p5 g_nm">
                            <div class="pv10 ph15 fs14  round3x sh10 cb60 tablinks bgw" id="{{ $category->id }}"
                                data-index=@php echo($counter) @endphp>
                                {{ $category->name }}
                            </div>
                        </div>
            ...
            @foreach ($categories as $category)
                @if ($loop->first)
                    <div class="fs0 @if ($business->lang == 'ar') r rtl @else l @endif c tabcontent mt0 active"
                        id="category{{ $category->id }}">
                    @else
                        <div class="fs0 @if ($business->lang == 'ar') r rtl @else l @endif c tabcontent "
                            id="category{{ $category->id }}">
                @endif
                    ...
                    <div class="g_nm w25 md_w33 m_w50 vt m_m_w100 p10 round4x  ">
                        <div class="bh10 bgw sh10 round4x">
                            <div class="w100 p10  fs14">
                                <tbl_nm>
                                    <cl class="w30 ">
                                        <div class="cov  w100 sq100 round2x"
                                            style="background-image: url('http://www.qr.algorexe.com/images/{{ $product->image }}');">
                                        </div>
                                    ...
                                    <cl class="w70 @if ($business->lang == 'ar') r pr10 @else l pl10 @endif">
                                        ...
                                        <div class="fs0 mt10 ">
                                            ...
                                            <div class="ov @if ($business->lang == 'ar') ra @else la @endif">
                                                <div class=" fs12 b g_nm round4x pointer ph10 pv5 bgb10 basket"
                                                    id="{{ $product->id }}">
                                                    @if ($business->lang == 'ar')
                                                        <span class="">إضافة إلى السلة</span>
                                                        <span class="l mdi mdi-basket"></span>
                                                    @else
                                                        <span class="">Add To Basket</span>
                                                        <span class="r mdi mdi-basket"></span>
                                                    @endif
                                                </div>
                                            </div>
                                    ...
                                    @if ($product->garnishes->isNotEmpty())
                                        @foreach ($product->garnishes as $garnish)
                                            <div class="l  fs0 mt5" style="border-bottom: 1px;">
                                                <div
                                                    class="fs14 b g_nm w50 @if ($business->lang == 'ar') r @else l @endif">
                                                    {{ $garnish->name }}</div>
                                                <div
                                                    class="fs14 b g_nm w50 @if ($business->lang == 'ar') l @else r @endif">
                                                    {{ $garnish->price }}
    ...
    <script src="{{ asset('js/yummy-bites/sw.js') }}"></script>
    ...
    <script>
        ...
        var garnish = document.querySelectorAll(".basket");
